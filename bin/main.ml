...
(* module StringBuilder = struct exception ArgumentError of string
   ...
   print_endline "" in prompt_loop exit (cycler ()) database | word1 :: word2 ::
   _ -> if word1 = "CREATE" && word2 = "TABLE" then let columns =
   StringBuilder.third_onward command in let database = Database.insert_table
   database (List.nth command 2) columns in prompt_loop exit (cycler ())
   database else let () = error_message () in prompt_loop exit (cycler ())
   database | _ -> let () = error_message () in prompt_loop exit (cycler ())
   database
